...
def pattern_menu():
    ...
    print("\n")
    print("'done' (to test pattern).  'restart' (to restart pattern)       ")
...
class Add_entry:
    ...
    def __init__(self):
        ...
        clear()
        while True:
            self.user_date = str(input('date> MM/DD/YYYY '))
            try:
...
class Search():
    ...
    def time_spent(self):
        ...
        try:
            for self.line in self.works_log:
                ...
                if str(self.match_time) == self.search.group(2):
                    print(
    ...
    def exact_task(self):
        ...
        for self.line in self.works_log:
            ...
            if str(self.match_task).upper() == self.search.group(1).upper():
                print(
                    "\n" + "Task: " + self.search.group(1) + " " +
                    "\n" + "Date: " + self.search.group(2) + " " +
                    "\n" + "Duration: " + self.search.group(3) + "\n" * 2 +
                    ...
                    self.search.group(4) +
                    "\n" * 2)
        if self.match_task.upper() != self.search.group(1).upper():
            print("There is no task under that name, sorry! ")
    ...
    def by_duration(self):
        ...
        try:
            ...
            for self.line in self.works_log:
                ...
                if str(self.duration_str) == str(self.search.group(3)):
                    print(
                        "Task Name: " + self.search.group(1) + " " +
                        "\n" + "Task Date: " + self.search.group(2) + " " +
                        "\n" + "Task Duration:" + self.search.group(3) + " " +
    ...
    def by_pattern_none(self):
        try:
            while True:
                ...
                if self.user == "Y":
                    ...
                    if self.user == 'BOTH':
                        ...
                        for self.line in self.works_log:
                            self.search = re.search(
                                self.pattern, self.line, re.I | re.M)
                            if self.search:
                                print(self.search.group(0))
                        break
                if self.user == "N":
            ...
            print("Check your pattern there was an error")
