...
class Search():
    ...
    def time_spent(self):
        ...
        for self.line in self.works_log:
            self.search = re.search(
                r'^([a-z\s\']+),' + str(self.match_time), self.line, re.I)
            if self.search:
    ...
    def exact_task(self):
        ...
        for self.line in self.works_log:
            ...
            if self.search:
                print(
                    "\n" + "Task: " + self.match_task + " " +
                    "\n" + "Date: " + self.search.group(1) + " " +
                    "\n" + "Duration: " + self.search.group(2) + "\n" * 2 +
                    "notes: " +
                    self.search.group(3) +
                    "\n" * 2)
    ...
    def all_tasks(self):
        ...
        for self.line in self.works_log:
            self.search = re.search(
                r'^([a-z\s\']+)[^.]+(\d{4}-\d{2}-\d{2})', self.line, re.I
                )
            if self.search:
                print("Task Name: " + self.search.group(1) + "\n" +
    ...
    def by_duration(self):
        ...
        self.works_log = open('works_log.csv', 'r', encoding='utf-8')
        for self.line in self.works_log:
            self.search_time = re.search(
                r'^([a-z\s\']+)[^.]+' + str(self.duration_str),
                self.line, re.I)
            if self.search_time:
                print(self.search_time.group(1) + " " + str(self.duration_str))
